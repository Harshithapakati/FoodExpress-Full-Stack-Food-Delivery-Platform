...
console.log('Calculating frontend lint score...');

try {
  // Run ESLint and capture output, suppressing warnings via environment variable
  const output = execSync('npx eslint . --ext .js,.jsx -f json', { 
    encoding: 'utf8',
  ...
  // Check if it's just a warning or an actual failure
  if (err.stdout) {
    try {
